...
def enumerate_lan_gui(root):
    ...
    scan_button = ttk.Button(lan_window, text="Start Scan", command=lambda: start_lan_scan(scan_type_var.get(), lan_tree))
...
def start_lan_scan(scan_type, lan_tree):
    threading.Thread(target=lambda: run_lan_enumeration(scan_type, lan_tree), daemon=True).start()

def run_lan_enumeration(scan_type, lan_tree):
    cmd = ["nmap", "-oX", "-", "192.168.1.0/24"]
